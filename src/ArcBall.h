...
inline static GLfloat Vector3fDot(const Vector3fT* NewObj, const Vector3fT* v1)
{
    ...
    return (NewObj->s.X * v1->s.X) + (NewObj->s.Y * v1->s.Y) + (NewObj->s.Z
                    * v1->s.Z);
}
...
inline static GLfloat Vector3fLengthSquared(const Vector3fT* NewObj)
{
    ...
    return (NewObj->s.X * NewObj->s.X) + (NewObj->s.Y * NewObj->s.Y)
...
{
    NewObj->s.M00 = NewObj->s.M01 = NewObj->s.M02 = NewObj->s.M10
                    = NewObj->s.M11 = NewObj->s.M12 = NewObj->s.M20
                                    = NewObj->s.M21 = NewObj->s.M22 = 0.0f;
}
...
inline static GLfloat Matrix4fSVD(const Matrix4fT* NewObj, Matrix3fT* rot3, Matrix4fT* rot4)
{
    ...
    s = FuncSqrt(((NewObj->s.XX * NewObj->s.XX) + (NewObj->s.XY * NewObj->s.XY)
                    + (NewObj->s.XZ * NewObj->s.XZ) + (NewObj->s.YX
                    * NewObj->s.YX) + (NewObj->s.YY * NewObj->s.YY)
                    + (NewObj->s.YZ * NewObj->s.YZ) + (NewObj->s.ZX
                    * NewObj->s.ZX) + (NewObj->s.ZY * NewObj->s.ZY)
    ...
    if(rot3) //if pointer not null
    {
        ...
        n = 1.0f / FuncSqrt((NewObj->s.XX * NewObj->s.XX) + (NewObj->s.XY
                        * NewObj->s.XY) + (NewObj->s.XZ * NewObj->s.XZ));
        rot3->s.XX *= n;
        rot3->s.XY *= n;
        rot3->s.XZ *= n;

        n = 1.0f / FuncSqrt((NewObj->s.YX * NewObj->s.YX) + (NewObj->s.YY
                        * NewObj->s.YY) + (NewObj->s.YZ * NewObj->s.YZ));
        rot3->s.YX *= n;
        rot3->s.YY *= n;
        rot3->s.YZ *= n;

        n = 1.0f / FuncSqrt((NewObj->s.ZX * NewObj->s.ZX) + (NewObj->s.ZY
                        * NewObj->s.ZY) + (NewObj->s.ZZ * NewObj->s.ZZ));
        rot3->s.ZX *= n;
        rot3->s.ZY *= n;
        rot3->s.ZZ *= n;
    }
...
{
    protected:
        inline void imageToSphereCoords(const Point2fT * NewPt, Vector3fT * NewVec) const;
    ...
    public:
        ...
        //Mouse down
        void click(const int inX, const int inY);

        //Mouse drag, calculate rotation
        void drag(const int inX, const int inY);

        GLfloat * getTransform();
